...
#ifdef OUTPUT_FOLDER
	FILE *output_file = ((struct cpu_args *)args)->output_file;
#endif
...
#ifdef OUTPUT_FOLDER
	FILE *output_file = ((struct mmpaging_ld_args *)args)->output_file;
#endif
...
/* Read config */
	...
	if (argc != 3)
	{
		printf("Usage: os [path to configure file] [path to output file]\n");
		return 1;
	}
#else
	if (argc != 2)
...
#ifdef OUTPUT_FOLDER
	mm_ld_args->output_file = output_file;
#endif
